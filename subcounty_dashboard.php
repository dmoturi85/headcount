<?php
session_start();
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'Subcounty Head') {
...
// Fetch employees for this subcounty
$sql = "SELECT * FROM employees WHERE subcounty = ?";
...
.sidebar { width: 230px; height: 100vh; background: #2c3e50; position: fixed; top: 0; left: 0; padding: 20px; color: white; }
...
<div class="content">
    <h3>Subcounty Dashboard - <?= htmlspecialchars($subcounty) ?></h3>
    ...
    <div class="row my-4">
        ...
        <div class="col-md-2"><div class="card p-3"><canvas id="genderChart" height="180"></canvas></div></div>
        <div class="col-md-2"><div class="card p-3"><canvas id="disabilityChart" height="180"></canvas></div></div>
    </div>
...
function drawCharts(data) {
    ...
    new Chart(document.getElementById('genderChart'), {
        type: 'pie',
        data: { labels: Object.keys(genderCounts), datasets: [{ data: Object.values(genderCounts), backgroundColor: ['#3b82f6', '#ef4444'] }] },
        options: { plugins: { legend: { position: 'bottom' } }, responsive: true }
    });

    new Chart(document.getElementById('disabilityChart'), {
        type: 'doughnut',
        data: { labels: Object.keys(disabilityCounts), datasets: [{ data: Object.values(disabilityCounts), backgroundColor: ['#22c55e', '#facc15'] }] },
        options: { plugins: { legend: { position: 'bottom' } }, responsive: true }
    });
}
...
function exportCSV() {
    ...
    a.download = 'subcounty_report.csv';
    a.click();
}
